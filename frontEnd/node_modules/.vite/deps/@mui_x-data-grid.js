...
} from "./chunk-TOPIPKDF.js";
import {
  Box_default,
  Popper_default
} from "./chunk-M4NTVAVU.js";
import "./chunk-5INXAXYL.js";
import {
  createSvgIcon,
  init_utils,
  useId_default
} from "./chunk-FQ3STS2O.js";
import {
  Badge_default,
  ClickAwayListener,
  FocusTrap,
  createFilterOptions
} from "./chunk-W2C56HPH.js";
import "./chunk-UJS4R4QJ.js";
...
var useGridPrintExport = (apiRef2, props) => {
  ...
  const doc = React104.useRef(null);
  const previousGridState = React104.useRef(null);
  const previousColumnVisibility = React104.useRef({});
  React104.useEffect(() => {
    doc.current = ownerDocument(apiRef2.current.rootElementRef.current);
  }, [apiRef2]);
  const updateGridColumnsForPrint = React104.useCallback((fields, allColumns) => new Promise((resolve) => {
  ...
  const handlePrintWindowLoad = React104.useCallback((printWindow, options) => {
...
`;
  ...
  const handlePrintWindowAfterPrint = React104.useCallback((printWindow) => {
  ...
  const exportDataAsPrint = React104.useCallback(async (options) => {
  ...
  const addExportMenuButtons = React104.useCallback((initialValue, options) => {
...
var useGridFilter = (apiRef2, props) => {
  ...
  const updateFilteredRows = React105.useCallback(() => {
    apiRef2.current.setState((state) => {
  ...
  const addColumnMenuItem = React105.useCallback((columnMenuItems, colDef) => {
    if (colDef == null || colDef.filterable === false || props.disableColumnFilter) {
      return columnMenuItems;
    }
    return [...columnMenuItems, "columnMenuFilterItem"];
  }, [props.disableColumnFilter]);
  const applyFilters = React105.useCallback(() => {
    updateFilteredRows();
    apiRef2.current.forceUpdate();
  }, [apiRef2, updateFilteredRows]);
  const upsertFilterItem = React105.useCallback((item) => {
  ...
  const upsertFilterItems = React105.useCallback((items2) => {
    const filterModel2 = gridFilterModelSelector(apiRef2);
  ...
  const deleteFilterItem = React105.useCallback((itemToDelete) => {
    const filterModel2 = gridFilterModelSelector(apiRef2);
  ...
  const showFilterPanel = React105.useCallback((targetColumnField, panelId, labelId) => {
  ...
  const hideFilterPanel = React105.useCallback(() => {
    logger.debug("Hiding filter panel");
    apiRef2.current.hidePreferences();
  }, [apiRef2, logger]);
  const setFilterLogicOperator = React105.useCallback((logicOperator) => {
    const filterModel2 = gridFilterModelSelector(apiRef2);
  ...
  const setQuickFilterValues = React105.useCallback((values) => {
    const filterModel2 = gridFilterModelSelector(apiRef2);
  ...
  const setFilterModel = React105.useCallback((model, reason) => {
  ...
  const preferencePanelPreProcessing = React105.useCallback((initialValue, value) => {
  ...
  const flatFilteringMethod = React105.useCallback((params) => {
  ...
  const handleColumnsChange = React105.useCallback(() => {
  ...
  const handleStrategyProcessorChange = React105.useCallback((methodName) => {
    if (methodName === "filtering") {
      apiRef2.current.unstable_applyFilters();
    }
  }, [apiRef2]);
  const updateVisibleRowsLookupState = React105.useCallback(() => {
    apiRef2.current.setState((state) => {
      return _extends({}, state, {
...
var useGridFocus = (apiRef2, props) => {
  ...
  const lastClickedCell = React106.useRef(null);
  const publishCellFocusOut = React106.useCallback((cell, event) => {
    if (cell) {
  ...
  const setCellFocus = React106.useCallback((id, field) => {
    const focusedCell = gridFocusCellSelector(apiRef2);
  ...
  const setColumnHeaderFocus = React106.useCallback((field, event = {}) => {
  ...
  const setColumnHeaderFilterFocus = React106.useCallback((field, event = {}) => {
    const cell = gridFocusCellSelector(apiRef2);
    publishCellFocusOut(cell, event);
    apiRef2.current.setState((state) => {
  ...
  const setColumnGroupHeaderFocus = React106.useCallback((field, depth, event = {}) => {
    const cell = gridFocusCellSelector(apiRef2);
    if (cell) {
      apiRef2.current.publishEvent("cellFocusOut", apiRef2.current.getCellParams(cell.id, cell.field), event);
    }
  ...
  const getColumnGroupHeaderFocus = React106.useCallback(() => unstable_gridFocusColumnGroupHeaderSelector(apiRef2), [apiRef2]);
  const moveFocusToRelativeCell = React106.useCallback((id, field, direction) => {
  ...
  const handleCellDoubleClick = React106.useCallback(({
    id,
    field
  }) => {
  ...
  const handleColumnHeaderFocus = React106.useCallback(({
    field
  }, event) => {
  ...
  const handleColumnGroupHeaderFocus = React106.useCallback(({
    fields,
    depth
  }, event) => {
    if (event.target !== event.currentTarget) {
  ...
  const handleBlur = React106.useCallback((_, event) => {
  ...
  const handleCellMouseDown = React106.useCallback((params) => {
    lastClickedCell.current = params;
  }, []);
  const handleDocumentClick = React106.useCallback((event) => {
  ...
  const handleCellModeChange = React106.useCallback((params) => {
  ...
  const handleRowSet = React106.useCallback(() => {
    const cell = gridFocusCellSelector(apiRef2);
...
var useGridKeyboardNavigation = (apiRef2, props) => {
  ...
  const theme = useTheme();
  const currentPageRows = React107.useMemo(() => enrichPageRowsWithPinnedRows(apiRef2, initialCurrentPageRows), [apiRef2, initialCurrentPageRows]);
  const headerFilteringEnabled = (
    // @ts-expect-error // TODO move relevant code to the `DataGridPro`
    props.signature !== "DataGrid" && props.unstable_headerFilters
  );
  const goToCell = React107.useCallback((colIndex, rowId, closestColumnToUse = "left") => {
  ...
  const goToHeader = React107.useCallback((colIndex, event) => {
  ...
  const goToHeaderFilter = React107.useCallback((colIndex, event) => {
  ...
  const goToGroupHeader = React107.useCallback((colIndex, depth, event) => {
    logger.debug(`Navigating to header col ${colIndex}`);
    apiRef2.current.scrollToIndexes({
      colIndex
    });
  ...
  const getRowIdFromIndex = React107.useCallback((rowIndex) => {
    return currentPageRows == null ? void 0 : currentPageRows[rowIndex].id;
  }, [currentPageRows]);
  const handleColumnHeaderKeyDown = React107.useCallback((params, event) => {
  ...
  const handleHeaderFilterKeyDown = React107.useCallback((params, event) => {
    const dimensions = apiRef2.current.getRootDimensions();
    if (!dimensions) {
      return;
    }
  ...
  const handleColumnGroupHeaderKeyDown = React107.useCallback((params, event) => {
    const dimensions = apiRef2.current.getRootDimensions();
    if (!dimensions) {
      return;
    }
  ...
  const checkIfCanStartEditing = React107.useCallback((initialValue, {
    event
  }) => {
    if (event.key === " ") {
      return false;
